--[[
    maddy.win | Da Hood (Overwrite Button Update)
    - Save Config: Safe mode (Won't delete existing files).
    - Overwrite Config: New button to force update a file.
]]
...
local Global = {
    Aimbot = false, AimPart = "HumanoidRootPart",
    SilentAim = false, SilentPart = "Closest Part",
    Prediction = 0.135, Smoothness = 0.1,
    TriggerBot = false, TriggerDelay = 0,
    DrawFOV = false, FOVRadius = 100,
    BoxEsp = false
}
...
local function GetConfigFiles()
    local Files = {}
    if listfiles then
        for _, FilePath in pairs(listfiles(ConfigFolder)) do
            local FileName = FilePath
            if FileName:find("/") then FileName = FileName:match("^.+/(.+)$") elseif FileName:find("\\") then FileName = FileName:match("^.+\\(.+)$") end
            if FileName:sub(-5) == ".json" then FileName = FileName:sub(1, -6); table.insert(Files, FileName) end
        end
    end
...
ConfigTab:CreateTextBox({ Name = "Config Name", Placeholder = "Type name...", Callback = function(val) 
    CurrentConfigName = val
    if UI.WarningLbl then UI.WarningLbl:Set(" ", Color3.new(1,1,1)) end
end })

UI.WarningLbl = ConfigTab:CreateLabel(" ")

ConfigTab:CreateButton({ Name = "Save Config", Callback = function()
    if CurrentConfigName == "" then UI.WarningLbl:Set("Enter a name!", Color3.fromRGB(255, 50, 50)) return end
    
    local FilePath = ConfigFolder .. "/" .. CurrentConfigName .. ".json"
    if isfile(FilePath) then
        UI.WarningLbl:Set("Exists! Use Overwrite button.", Color3.fromRGB(255, 100, 0))
        return
    end
    
    local Json = HttpService:JSONEncode(Global)
    writefile(FilePath, Json)
    UI.WarningLbl:Set("Saved successfully!", Color3.fromRGB(100, 255, 100))
    UI.ConfigList:Refresh(GetConfigFiles())
    task.wait(2)
    UI.WarningLbl:Set(" ", Color3.new(1,1,1))
end })

-- NEW OVERWRITE BUTTON
ConfigTab:CreateButton({ Name = "Overwrite Config", Callback = function()
    if CurrentConfigName == "" then UI.WarningLbl:Set("Enter a name!", Color3.fromRGB(255, 50, 50)) return end
    
    local FilePath = ConfigFolder .. "/" .. CurrentConfigName .. ".json"
    -- No check for existence, just write
    local Json = HttpService:JSONEncode(Global)
    writefile(FilePath, Json)
    
    UI.WarningLbl:Set("Overwritten successfully!", Color3.fromRGB(100, 255, 100))
    UI.ConfigList:Refresh(GetConfigFiles())
    task.wait(2)
    UI.WarningLbl:Set(" ", Color3.new(1,1,1))
end })
...
ConfigTab:CreateButton({ Name = "Load Selected", Callback = function()
    ...
    if isfile(Path) then
        ...
        UI.WarningLbl:Set("Loaded: " .. SelectedConfig, Color3.fromRGB(100, 255, 100))
    end
end })

ConfigTab:CreateButton({ Name = "Refresh List", Callback = function() UI.ConfigList:Refresh(GetConfigFiles()) end })
...
RunService.RenderStepped:Connect(function()
    ...
    if Global.TriggerBot then
        ...
        if t and t.Parent and t.Parent:FindFirstChild("Humanoid") and t.Parent.Name ~= LocalPlayer.Name then
            task.wait(Global.TriggerDelay/1000); mouse1click()
        end
    end
    ...
    if Global.Aimbot or Global.SilentAim then
        ...
        if CurrentTarget and CurrentTarget.Character then
            ...
            elseif Global.Aimbot then Part = CurrentTarget.Character:FindFirstChild(Global.AimPart) end
